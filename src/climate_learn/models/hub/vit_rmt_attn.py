...
from timm.models.vision_transformer import trunc_normal_, DropPath
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, in_chans=3, embed_dim=96, patch_size=2, norm_layer=None, bias=True):
        super().__init__()
        self.in_chans = in_chans
        self.embed_dim = embed_dim

        self.proj = nn.Sequential(
            nn.Conv2d(in_chans, embed_dim, patch_size, patch_size, bias=bias),
        )
...
@register("vit_rmt_attn")
class RetentiveMeetVisionTransformer(nn.Module):
    def __init__(
        self,
        img_size,
        in_channels,
        ...
        self.patch_embed = PatchEmbed(in_chans=in_channels, embed_dim=embed_dims[0], patch_size=patch_size,
        ...
        for _ in range(decoder_depth):
            self.head.append(nn.Linear(embed_dims[0], embed_dims[0]))
            self.head.append(nn.GELU())
        self.head.append(nn.Linear(embed_dims[0], out_channels * patch_size**2))
