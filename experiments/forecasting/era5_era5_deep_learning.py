...
direct.add_argument("--model", choices=["resnet", "unet", "vit", "vit_swin_attn", "vit_primal_attn", "vit_rmt_attn", "mlp_mixer", "mlp_temp_mixer","vit_temporal"])
...
elif args.model == "mlp_temp_mixer":
    model_kwargs = {  # override some of the defaults
        "img_size": (32, 64),
        "in_channels": in_channels,
        "out_channels": out_channels,
...
logger = TensorBoardLogger(save_dir=f"savelog/{default_root_dir}/logs")
...
callbacks = [
    ...
    ModelCheckpoint(
        dirpath=f"savelog/{default_root_dir}/checkpoints",
